...
const OrderModal = ({ isOpen, onClose }) => {
    ...
    const [selectedMenuItem, setSelectedMenuItem] = useState('');
    const [currentItemQuantity, setCurrentItemQuantity] = useState(1);
    const [taxRate, setTaxRate] = useState(2.5); // State for tax rate input
    ...
    const calculateTotal = () => {
        const subTotal = items.reduce((total, item) => total + item.price, 0);
        const taxAmount = subTotal * (taxRate / 100); // Calculate tax based on user input percentage
